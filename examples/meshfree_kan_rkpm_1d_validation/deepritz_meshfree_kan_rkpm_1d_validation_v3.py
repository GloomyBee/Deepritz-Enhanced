...
def train_phase_a(
    ...
    history = {"steps": [], "loss": [], "linear": [], "pu": [], "bd": []}
    ...
    for step in range(steps):
        ...
        # Boundary anchors for endpoints:
        # phi_0(0)=1, phi_i>0(0)=0, phi_{N-1}(1)=1, phi_i<N-1(1)=0
        phi_raw_bd = model.compute_shape_functions(x_bd, return_raw=True)
        phi_sum_bd = torch.sum(phi_raw_bd, dim=1, keepdim=True) + 1e-12
        phi_bd = phi_raw_bd / phi_sum_bd
        ...
        loss = loss_linear + lambda_pu * loss_pu + lambda_bd * loss_bd
        ...
        if step % log_interval == 0 or step == steps - 1:
            ...
            print(
                ...
                f"Linear={loss_linear.item():.3e} | PU={loss_pu.item():.3e} | BD={loss_bd.item():.3e}"
            )
...
def main():
    ...
    parser.add_argument("--lambda-bd", type=float, default=1.0, help="Boundary anchor penalty weight")
    ...
    metrics = {
        ...
        "lr": float(args.lr),
        "lambda_pu": float(args.lambda_pu),
