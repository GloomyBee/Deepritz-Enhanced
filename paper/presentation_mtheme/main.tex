...
\vspace{1em}
\begin{itemize}
  \item Phase A：先学习 shape function 的几何一致性
  \item Phase B：在固定或联合优化下进入 Poisson 能量最小化
  \item fallback、teacher、regularization 是关键稳定化机制
\end{itemize}
\smallnote{方法细节可对照公共模块与论文主文档。}
\end{frame}
...
\vspace{0.4em}
\begin{itemize}
  \item patch test 检验的是：$\sum_I \phi_I \approx 1$ 与一次重构是否成立
  \item 这一步不是主 PDE 结果，而是 shape function 合法性验证
  \item \texttt{PU residual}、\texttt{linear reproduction} 与 $\Lambda_h$ 一起报告更完整
\end{itemize}
\end{frame}
...
\column{0.54\textwidth}
\begin{itemize}
  \item 问题：\texttt{sinusoidal\_poisson\_2d}
  \item 网格规模：\texttt{n\_side = 11, 15, 21}
  \item 当前主变体：\texttt{no\_softplus\_teacher\_reg}
  \item 关键参数：\texttt{kappa=2.5}, \texttt{beta\_bc=500}, \texttt{gamma\_linear=1}
\end{itemize}
\column{0.42\textwidth}
\begin{block}{目标}
让 2D \texttt{L2/H1} 收敛曲线更接近论文投稿所需的展示质量，同时保持 \texttt{PU} 与 $\Lambda_h$ 的稳定。
\end{block}
\end{columns}
\smallnote{当前最佳结果对应 \texttt{paper\_v3\_main} 系列输出目录。}
\end{frame}
